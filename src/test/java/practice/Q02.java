...
public class Q02 extends ReqresBaseUrl {
    ...
    Given
            ...
            2) {
        "name": "morpheus",
                "job": "leader"           postman'de POST request yaptik
    }
    When
    ...
    public void post() {
        ...
        Response response = given().log().all().spec(spec).contentType(ContentType.JSON).body(expectedData).when().post("/{first}/{second}");
